---
title: "Analysis of Set7KD in endothelial cells"
author: "Mark Ziemann"
date: "`r Sys.Date()`"
output:
  html_document:
...
In this script I will investigate the effect of knock-down of Set7 lysine methyltransferase
in endothelial cells.
The data is available from NCBI GEO GSE93236.
...
Change the underscore to a space in the rownames.
...
ss <- as.data.frame(colnames(mx))

ss$trt <- as.numeric(grepl("Set7KD",colnames(x)))

ss %>%
  kbl(caption="sample sheet for Set7 knock down in endothelial cells") %>%
...
par(mar=c(5,7,5,1))
barplot(rev(colSums(mx)),horiz=TRUE,las=1,main="number of reads per sample in GSE93236")
...
cols <- as.character(grepl("Set7KD",colnames(mx)))
...
head(def,10) %>%
  kbl(caption="Top DE genes for Set7 knock-down") %>%
  kable_paper("hover", full_width = F)
...
plot(log10(def$baseMean) ,def$log2FoldChange,
  ...
  main="Effect of Set7KD in endothelial cells",
...
| Data availability | Data obtained from NCBI GEO under accession GSE93236 |
